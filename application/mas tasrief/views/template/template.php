...
<head>
	...
	<link rel="stylesheet" type="text/css"
		href="<?= base_url('assets/assetsegbis/js/leaflet-search/dist/leaflet-search.min.css') ?>">
	<link rel="stylesheet" href="https://unpkg.com/leaflet@1.3.4/dist/leaflet.css"
		integrity="sha512-puBpdR0798OZvTTbP4A8Ix/l+A4dHDD0DGqYW6RQ+9jxkRFclaxxQb/SJAWZfWAkuyeQUytO7+7N4QKrDh+drA=="
		crossorigin="" />
	<link rel="stylesheet"
		href="<?= base_url() ?>assets/assetsegbis/js/leaflet-panel-layers-master/src/leaflet-panel-layers.css" />
	<style type="text/css">
		#map {
			height: 100vh;
		}
  ...
  // DropzoneJS Demo Code End
	...
	<script type="text/javascript">
		...
		$(function () {
			...
			$('#kategorifilterado').change(function () {
				var kat = $(this).val();
				if (kat == 'BANDARA') { $(".bandaracolor").css("background-color", "#dedede"); } else { $(".bandaracolor").css("background-color", "#fff"); }

				if (kat == 'HOTEL') { $(".hotelcolor").css("background-color", "#dedede"); } else { $(".hotelcolor").css("background-color", "#fff"); }

				if (kat == 'RESTAURANT') { $(".restaurantcolor").css("background-color", "#dedede"); } else { $(".restaurantcolor").css("background-color", "#fff"); }

				if (kat == 'APARTEMEN') { $(".apartemencolor").css("background-color", "#dedede"); } else { $(".apartemencolor").css("background-color", "#fff"); }

				if (kat == 'SEKOLAH') { $(".sekolahcolor").css("background-color", "#dedede"); } else { $(".sekolahcolor").css("background-color", "#fff"); }

				if (kat == 'FASKES') { $(".faskescolor").css("background-color", "#dedede"); } else { $(".faskescolor").css("background-color", "#fff"); }

				if (kat == 'INDUSTRI SEDANG') { $(".industrisedangcolor").css("background-color", "#dedede"); } else { $(".industrisedangcolor").css("background-color", "#fff"); }

				if (kat == 'GOVT OFFICE') { $(".govtofficecolor").css("background-color", "#dedede"); } else { $(".govtofficecolor").css("background-color", "#fff"); }

				if (kat == 'KAWASAN EKONOMI KHUSUS') { $(".kekcolor").css("background-color", "#dedede"); } else { $(".kekcolor").css("background-color", "#fff"); }

				if (kat == 'MALL') { $(".mallcolor").css("background-color", "#dedede"); } else { $(".mallcolor").css("background-color", "#fff"); }

				if (kat == 'INDUSTRI BESAR') { $(".industribesarcolor").css("background-color", "#dedede"); } else { $(".industribesarcolor").css("background-color", "#fff"); }

				if (kat == 'INDUSTRI KECIL') { $(".industrikecilcolor").css("background-color", "#dedede"); } else { $(".industrikecilcolor").css("background-color", "#fff"); }

				if (kat == 'UNIVERSITAS') { $(".universitascolor").css("background-color", "#dedede"); } else { $(".universitascolor").css("background-color", "#fff"); }

				if (kat == 'KAWASAN INDUSTRI') { $(".kawasanindustricolor").css("background-color", "#dedede"); } else { $(".kawasanindustricolor").css("background-color", "#fff"); }

				if (kat == 'BUMN') { $(".bumncolor").css("background-color", "#dedede"); } else { $(".bumncolor").css("background-color", "#fff"); }
	...
	<script
		src="<?= base_url('assets/assetsegbis/js/leaflet-panel-layers-master/src/leaflet-panel-layers.js') ?>"></script>
	...
	<script>
		...
		$(document).ready(function () {
			...
			// modal show user
			$('#modal-default').on('show.bs.modal', e => {
				var $button = $(e.relatedTarget);
				$('#modal_username').val($button.closest('td').prev().prev().prev().text().trim());
				$('#modal_witel').val($button.closest('td').prev().prev().text().trim());
				$('#modal_nama').val($button.closest('td').prev().prev().prev().prev().text().trim());
				$('#modal_last_login').val($button.closest('td').prev().text().trim());

			});

			// modal show log
			$('#modal-log').on('show.bs.modal', e => {
				var $button = $(e.relatedTarget);
				$('#modal_cc').val($button.closest('td').prev().prev().prev().prev().text().trim());
				$('#modal_waktu').val($button.closest('td').prev().prev().text().trim());
				$('#modal_witel').val($button.closest('td').prev().prev().prev().text().trim());
				$('#modal_modifier').val($button.closest('td').prev().prev().prev().prev().prev().text().trim());
				$isi = $button.closest('.attrib-modal').find("input[name='log']").val();
				$('#log').append($isi);
			});
			...
			$('#modal-edit').on('show.bs.modal', e => {
				var $button = $(e.relatedTarget);
				$('#modal_edit_username').val($button.closest('td').prev().prev().prev().text().trim());
				$('#modal_edit_witel').val($button.closest('td').prev().prev().text().trim());
				$('#modal_edit_nama').val($button.closest('td').prev().prev().prev().prev().text().trim());
				$('#modal_edit_last_login').val($button.closest('td').prev().text().trim());
				$('#modal_edit_id_user').val($button.closest('.attrib-modal').find("input[name='id_user']").val());

			});
			// disable dan hilangkan password form
			$('#form-pass').hide();
			$('#edit_old').prop('disabled', true);
			$('#edit_new').prop('disabled', true);

			// cek rubah password atau tidak 
			$('#exampleCheck1').change(function () {
				if ($(this).is(":checked")) {
					$('#form-pass').show();
					$('#edit_old').prop('disabled', false);
					$('#edit_new').prop('disabled', false);
				} else {
					$('#form-pass').hide();
					$('#edit_old').prop('disabled', true);
					$('#edit_new').prop('disabled', true);
			...
			$('#modal_add_button').click(function () {
				// cek inputan diatas 6 karakter
				if ($('#modal_add_username').val().length >= 6) {

					// cekpassword panjang karakter
					if ($('#modal_add_password').val().length >= 8) {
						// cek kesamaan password
						if ($('#modal_add_password').val() == $('#modal_add_konfirmasi_password').val()) {
						...
						$(document).Toasts('create', {
							...
							autohide: true,
							delay: 1000,
							body: 'password harus lebih atau sama dengan 8'
						})
						return false;
					}
					...
					$(document).Toasts('create', {
						...
						autohide: true,
						delay: 1000,
						body: 'username harus lebih atau sama dengan 6'
					})
					return false;
				}
