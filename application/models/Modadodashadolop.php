...
class Modadodashadolop extends CI_Model
{
	var $table = 'dataado'; //nama tabel dari database
    var $column_order = array(null, 'am','witel','nama_cust','alamat','det_kat','odp','id_lop'); //field yang ada di table user
    var $column_search = array('am','witel','nama_cust','alamat','det_kat','odp','id_lop'); //field yang diizin untuk pencarian 
			...
			if ($hero=='All')
			{
				if ($datel=="All")
				{
					if ($witel=="All")
					{
						...
						$this->db->where("(id_lop<>'' and id_lop<>'-')", NULL, FALSE);
					}
					else
					{
						$this->db->where("kat", $kat);
